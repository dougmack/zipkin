  ...
  CassandraSpanConsumer(Session session, boolean strictTraceId) {
    ...
    insertSpan = session.prepare(
        QueryBuilder
            ...
            .value("local_endpoint", QueryBuilder.bindMarker("local_endpoint"))
            .value("local_service_name", QueryBuilder.bindMarker("local_service_name"))
            .value("remote_endpoint", QueryBuilder.bindMarker("remote_endpoint"))
  ...
  void storeSpan(Span span, long timestamp) {
    try {
      ...
      if (null != span.localEndpoint()) {
        bound = bound
                .set("local_endpoint", new EndpointUDT(span.localEndpoint()), EndpointUDT.class)
                .setString("local_service_name", span.localServiceName());
      }
      if (null != span.remoteEndpoint()) {
        bound = bound.set("remote_endpoint", new EndpointUDT(span.remoteEndpoint()), EndpointUDT.class);
      }
      if (null != span.shared()) {
